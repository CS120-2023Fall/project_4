...
#include <tchar.h>
static uint16_t TOTAL_PACKET_LEN = 74;
...
static void calculate_total_length(const u_char *packetData) {
    unsigned int byte0;
    unsigned int byte1;
...
static void get_the_information_of_the_packet(u_char* packetData, ip_header* ip_layer_head, icmp_header* icmp_layer_head) {
    ip_layer_head = (struct ip_header*)(packetData + 14);
    icmp_layer_head = (struct icmp_header*)(packetData + 14 + 20);

}
static void calculate_check_sum_ip(u_char* packetData, unsigned int packet_len) {
    uint32_t cksum = 0;
...
static void calculate_check_sum_ICMP(u_char* packetData, unsigned int packet_len) {
    uint32_t cksum = 0;
...
static int PrintICMPHeader(const u_char* packetData) {
...
static int PrintIPHeader(const u_char* packetData) {
